...
The other for the accumulated denominator of the softmax equation
$$
l = \{l_i\}_{i = 1}^N \\
...
$$
o_i' = \sum_{j = 1}^{i - 1} \frac{e^{x_j - m_i}}{l_i} V[j, :] + \frac{e^{x_i - m_i}}{l_i} V[i, :] = o_{i - 1}' \frac{l_{i - 1} e^{m_{i - 1} - m_i}}{l_i} + \frac{e^{x_i - m_i}}{l_i} V[i, :]
$$
...
\frac{1}{y_i}\frac{\partial y_i}{\partial x_j} = \frac{\partial x_i}{\partial x_j} - \frac{\frac{\sum_{k = 1}^N e^{x_k}}{\partial x_j}}{\sum_{k = 1}^N e^{x_k}} = \mathbb{I}[i = j] - y_j \\
...
$$
\frac{\partial L}{\partial x_i} = \sum_{j = 1}^N \frac{\partial L}{\partial y_j} \frac{\partial y_j}{\partial x_i} = \sum_{j = 1}^N \frac{\partial L}{\partial y_j} y_j (\mathbb{I}[i = j] - y_i) = dy \cdot J[:, i] \\
dx = dy \cdot J = dy (\text{diag}(y) - y^Ty)
$$
...
$$
ds = dp (\text{diag}(p) - p^Tp) = dp \odot p - (dp \cdot p^T) p
$$

We know that $dP = dO \cdot V^T$, which gives us

$$
dp = dP[i, :] = \sum_{j = 1}^N dO[i, :] V[:, j] \\
dp \cdot p^T = \sum_{j = 1}^N dO[i, :] V[:, j] P[i, :]^T = dO[i, :] \sum_{j = 1}^N V[:, j] P[i, :]^T = dO[i, :] O[i, :]^T = do \cdot o^T \in \mathbb{R}
$$
